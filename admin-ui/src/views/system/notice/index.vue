        ...
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="内容">
              <el-input :rows="6" type="textarea" placeholder="请输入内容" v-model="form.content" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
function handleUpdate(row) {
  ...
  noticeApi.getNotice(id).then((response) => {
    form.value = response;
    open.value = true;
    title.value = '修改公告';
  });
...
function handleDelete(row) {
  const noticeIds = row.id != null ? Array.of(row.id) : ids.value;
  proxy.$modal
